...
impl Scheme for Scheme02 {
    fn hash(&self, to_hash: &crate::pwd::ContentToHash) -> Result<String, PwdError> {
        let argon2 = get_argon2();

        let salt_b64 = SaltString::encode_b64(to_hash.salt.as_bytes())
            .map_err(|_| PwdError::SchemeError(SchemeError::Salt))?;

        let pwd = argon2
            .hash_password(to_hash.content.as_bytes(), &salt_b64)
            .map_err(|_| PwdError::SchemeError(SchemeError::Hash))?
            .to_string();

        Ok(pwd)
    }

    fn validate(&self, to_hash: &crate::pwd::ContentToHash, pwd_ref: &str) -> Result<(), PwdError> {
        let argon2 = get_argon2();

        let parsed_hash_ref =
            PasswordHash::new(pwd_ref).map_err(|_| PwdError::SchemeError(SchemeError::Hash))?;

        argon2
            .verify_password(to_hash.content.as_bytes(), &parsed_hash_ref)
            .map_err(|_| PwdError::SchemeError(SchemeError::PwdValidate))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_scheme_02_hash_into_b64u_ok() {
        // -- Setup & Fixtures
        ...
        // -- Exec
        let scheme = Scheme02;
        let res = scheme.hash(&fx_to_hash).unwrap();
