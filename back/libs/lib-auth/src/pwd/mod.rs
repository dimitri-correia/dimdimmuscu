...
pub async fn hash_pwd(to_hash: ContentToHash) -> Result<String, PwdError> {
...
fn hash_for_scheme(scheme_name: &str, to_hash: ContentToHash) -> Result<String, PwdError> {
...
impl FromStr for PwdParts {
    ...
    fn from_str(pwd_with_scheme: &str) -> Result<Self, PwdError> {
        regex_captures!(
            r#"^#(\w+)#(.*)"#, // a literal regex
            pwd_with_scheme
        )
...
mod tests {
    ...
    async fn test_multi_scheme_ok() {
        ...
        // -- Exec
        let pwd_hashed = hash_for_scheme("01", fx_to_hash.clone()).unwrap();
        let pwd_validate = validate_pwd(fx_to_hash.clone(), pwd_hashed).await.unwrap();
